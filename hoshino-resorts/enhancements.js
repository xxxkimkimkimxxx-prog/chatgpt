...
サーフジャック ハワイ~個性的な宿~海外~米国・ハワイ ワイキキ~2~海・水辺,街歩き,海外,海~営業中~0`;
  ...
  function enhanceCard(card){
    ...
    if(!a){a=document.createElement('a');a.className='officialDirect';a.target='_blank';a.rel='noopener noreferrer';a.textContent='公式サイトへ ↗';const detail=card.querySelector('.detailBtn');(detail||card).before?detail.before(a):card.appendChild(a);}
  ...
  const ct=document.getElementById('compareTable');if(ct)new MutationObserver(run).observe(ct,{childList:true,subtree:true});
